...
Derek Martin
CIS 211
Contains Appt and Agenda classes
'''
...
class Appt:
    ...
    Usage example:
        ...
        if appt 2 > appt 1:
            print(f"appt1 '{appt1}' was over when appt '{appt}' started")
    ...
    def __eq__(self, other: "Appt") -> bool:
        '''Equal if they have the same start and finish times'''
    ...
    def intersect(self, other: "Appt") -> "Appt":
        if self.overlaps(other):
            if self.start > other.start and self.start < other.finish:
                return Appt(self.start, other.finish, f"({self.desc}) and ({other.desc})")
            elif other.start > self.start and other.start < self.finish:
                return Appt(other.start, self.finish, f"{other.desc} and {self.desc}")
        ...
        for i in self:
            ...
            for j in self[ctr:]:
                if j.start > i.finish:
                    break
                if j.overlaps(i):
                    conflictions.append(i.intersect(j))
        return conflictions
